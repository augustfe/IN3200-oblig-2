                             ...
                             double **my_u, double **my_u_prev)
{
    ...
    for (i = 1; i < my_ny + (has_neigh_above * has_neigh_below); i++)
    {
        ...
        for (j = 1; j < nx - 1; j++)
        {
            ...
            my_u_new[i][j] = 2 * my_u[i][j] + dx_const * (u_left - 2 * my_u[i][j] + u_right) +
                             dy_const * (u_below - 2 * my_u[i][j] + u_above) - my_u_prev[i][j];
        }
        j = nx - 1;
        my_u_new[i][j] = 2 * my_u[i][j] + dx_const * (2 * my_u[i][j - 1] - 2 * my_u[i][j]) +
    ...
    if (my_rank == 0)
    {
        ...
        for (j = 1; j < nx - 1; j++)
        {
            ...
            my_u_new[i][j] = 2 * my_u[i][j] + dx_const * (u_left - 2 * my_u[i][j] + u_right) +
                             dy_const * (2 * u_below - 2 * my_u[i][j]) - my_u_prev[i][j];
        }
        // Upper corners
    ...
    else if (my_rank == P - 1)
    {
        ...
        // Lower corners
        my_u_new[i][0] = 2 * my_u[i][0] + dx_const * (2 * my_u[i][1] - 2 * my_u[i][0]) +
                         dy_const * (2 * my_u[i - 1][0] - 2 * my_u[i][0]) - my_u_prev[i][0];

        my_u_new[i][nx - 1] = 2 * my_u[i][nx - 1] + dx_const * (2 * my_u[i][nx - 2] - 2 * my_u[i][nx - 1]) +
                              dy_const * (2 * my_u[i - 1][nx - 1] - 2 * my_u[i][nx - 1]) - my_u_prev[i][nx - 1];
    }
}